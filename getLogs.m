function getLogs()
    ...
    while numTotal ~= 0       
        ...
        if num ~= 0
            ...
            system(r,['rm ',name,'*']);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Data = dlmread(['log','_',num2str(num_flight),'.txt']); %Use a better name that M
            logs_generated=dir([pathWorkspace,nameProjectFolder,'Logs/*.mat']);
            num_logs_generated=size(logs_generated,1);
            %name_new = [nameModelSimulink,'_',num2str(num_logs_generated+1),'_',datetime('today')];
